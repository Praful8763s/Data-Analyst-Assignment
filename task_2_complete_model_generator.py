"""
TASK 2: Complete Dynamic Profitability Model Generator
Creates a fully functional Excel model with all formulas populated and calculated
"""
...
outputs = [
    ...
    ("Total Organic Revenue", f"=B20*{Org_units_cell}", "₹"),
...
for idx, (metric, formula, unit) in enumerate(outputs, 1):
    ...
    ws[f'D{current_row}'] = formula
    ws[f'D{current_row}'].font = Font(size=9, color="595959")
...
health_checks = [
    ("Profit Status", f"=IF(B25>0,\"Profitable\",\"Loss\")", "B25 should be positive"),
    ("ACOS Status", f"=IF(B28<0.30,\"Excellent\",IF(B28<0.35,\"Good\",\"Needs Work\"))", "Aim for <30%"),
    ...
    ("Organic Growth Signal", f"=IF({Org_units_cell}>{Ad_units_cell},\"Strong\",\"Weak\")", "Organic should grow"),
]
...
# Base Case (references Profitability Model sheet)
base_case_col = 2
base_formulas = {
...
print(f"SUCCESS: Excel model created at {output_path}")
print()
print("Model Structure:")
print("  Sheet 1: 'Instructions' - User guide")
print("  Sheet 2: 'Profitability Model' - Main calculation engine")
print("  Sheet 3: 'Scenario Analysis' - 5 scenarios pre-built")
print()
print("Input cells (gray): Column B in Profitability Model sheet")
print("Output cells (blue): All calculated automatically with formulas")
print()
print("Ready to use! Edit gray INPUT cells and all calculations update.")
